...
int main(int argc, char **argv) {
    ...
    /* Variables and flags */
    char *interface = NULL;
    string filter = "tcp or udp or arp or icmp or icmp6 or igmp";
    int udp = 0;
    int tcp = 0;
    int arp = 0;
    int icmp = 0;
    ...
    static struct option long_options[] = {
        {"udp", no_argument, &udp, 1},    {"tcp", no_argument, &tcp, 1},
        {"arp", no_argument, &arp, 1},    {"icmp", no_argument, &icmp, 1},
        {"igmp", no_argument, &igmp, 1},  {"interface", optional_argument, NULL, 'i'},
    ...
    if (udp + tcp + arp + icmp + igmp + port > 0) {
        filter = set_filter(port, tcp, udp, icmp, arp, igmp);
    }
...
void print_help() {
    ...
    printf("Usage:");
    printf("  ./ipk-sniffer [-i interface | --interface interface] {-p port} {[--tcp|-t] [--udp|-u] [--arp] [--icmp] [--igmp]} {-n packet_cnt}\n\n");
    ...
    printf("    {[--tcp|-t] [--udp|-u] [--arp] [--icmp] [--igmp]} - Packet types filters\n");
